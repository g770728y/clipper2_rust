...
pub type CLong = libc::c_longlong;
...
mod ffi {
    ...
    pub type CPaths64 = *const CLong;
    pub type CPaths64Mut = *mut CLong;
    pub type CPaths64Ref = *mut *mut CLong;
    ...
    extern "C" {
        ...
        pub fn RectClip64(rect: *const CRect64, paths: CPaths64) -> CPaths64Mut;

        pub fn RectClipLines64(rect: *const CRect64, paths: CPaths64) -> CPaths64Mut;

        pub fn MinkowskiSum64(pattern: CPaths64, path: CPaths64, is_closed: bool) -> CPaths64Mut;

        pub fn MinkowskiDiff64(pattern: CPaths64, path: CPaths64, is_closed: bool) -> CPaths64Mut;
        ...
        pub fn TrimCollinear64(paths: CPaths64, is_open_path: bool) -> CPaths64Mut;
...
fn cpaths_from_vec<T>(paths: &Vec<Vec<T>>) -> Vec<CLong>
where
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    let n_paths = paths.len();
    let n_coords = paths.iter().map(|x| x.len()).sum::<usize>();
    let buffer_len = 2 * (n_paths + n_coords + 1);

    let mut cpaths_buffer = vec![0 as CLong; buffer_len];
...
fn cpaths_from_coords<T>(coords: &Vec<T>) -> Vec<CLong>
where
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
...
fn vec_from_cpaths<T>(cpaths_buffer: &Vec<CLong>) -> Vec<Vec<T>>
where
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    ...
    for _ in 0..n_paths {
        ...
        for _ in 0..path_len {
            // change when https://github.com/rust-lang/rust/issues/98326 has been closed?
            let point = [
...
unsafe fn vec_from_raw_cpaths<T>(cpaths_ptr: *mut *mut CLong) -> Vec<Vec<T>>
where
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
...
where
    R: Into<[CLong; 4]> + From<[CLong; 4]> + Copy,
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    ...
    let result = unsafe {
        *result_buffer = ffi::RectClip64(&crect, input_buffer.as_ptr());
...
pub fn rect_clip_lines_ext<R, T>(
    rect: &R,
    paths: &Vec<Vec<T>>,
) -> Result<Vec<Vec<T>>, RectClipError>
where
    R: Into<[CLong; 4]> + From<[CLong; 4]> + Copy,
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    ...
    let result = unsafe {
        *result_buffer = ffi::RectClipLines64(&crect, input_buffer.as_ptr());
    ...
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    ...
    let result = unsafe {
        *result_buffer =
            ffi::MinkowskiSum64(pattern_buffer.as_ptr(), path_buffer.as_ptr(), is_closed);
        vec_from_raw_cpaths(result_buffer)
    };
    ...
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    ...
    let result = unsafe {
        *result_buffer =
            ffi::MinkowskiDiff64(pattern_buffer.as_ptr(), path_buffer.as_ptr(), is_closed);
        vec_from_raw_cpaths(result_buffer)
    };
    ...
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    ...
    let result = unsafe {
        *result_buffer = ffi::SimplifyPaths64(input_buffer.as_ptr(), epsilon, is_open_path);
        vec_from_raw_cpaths(result_buffer)
    };
    ...
    T: Into<[CLong; 2]> + From<[CLong; 2]> + Copy,
{
    ...
    let mut result = unsafe {
        *result_buffer = ffi::TrimCollinear64(input_buffer.as_ptr(), is_open_path);
        vec_from_raw_cpaths(result_buffer)
    };
...
mod tests {
    ...
    fn rect_clip_test() {
        ...
        let circle_vertices: Vec<Vec<I64Vec2>> = vec![(0..n_points)
            .map(|i| I64Vec2::new(circle_x(i) as i64, circle_y(i) as i64))
            .collect()];
